...
uint256 hashGenesisBlock("0x0528cdd0b5a104fa0f095a66174777f2de57e13dbdf40557702deb7c123c48c5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Egypt: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Egypt Signed Message:\n";
...
{
    // Egypt: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Egypt: 10k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Egypt: 1 day
static const int64 nTargetSpacing = 5 * 60; // Egypt: 5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Egypt: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x808874736dd26182425295aa5f1942d3959a2599c0ed85ed5f2a88dd9371c660");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "EGYPT is GOLD";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ab2acd360cb396280007a9ea901f1e4af709da0c7abcdea8cd4a46126e61a082f314eae6fb4ec1d2a33562c2d6329830c9acc46fdf17982c3357c0cd40965404") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1545630546;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084743452;

        if (fTestNet)
        {
            block.nTime    = 1545630504;
            block.nNonce   = 385949702;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf27d9c9b4c44a66fff252ef842e5d852147245f46117ef35af1dfacd33159b18"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xc9, 0xbc, 0xd6 }; // Egypt: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EgyptMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EgyptMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EgyptMiner(CWallet *pwallet)
{
    printf("EgyptMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("egypt-miner");
    ...
    try { loop {
        ...
        printf("Running EgyptMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EgyptMiner, pwallet));
}
